...
const AdminPasswordModal: React.FC<AdminPasswordModalProps> = ({
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      console.log('🔑 Adding password to admin Google account...');
      ...
      if (hasPassword) {
        // Admin already has password, update it
        console.log('🔄 Updating existing admin password...');
        await updatePassword(auth.currentUser, newPassword);
        console.log('✅ Admin password updated successfully');
        setSuccess('Admin password updated successfully!');
      } else {
        // Admin doesn't have password, add it by linking
        console.log('🔗 Adding password provider to admin Google account...');
        const credential = EmailAuthProvider.credential(auth.currentUser.email || '', newPassword);
        await linkWithCredential(auth.currentUser, credential);
        console.log('✅ Admin password provider added successfully');
        setSuccess('Admin password added successfully! You can now login with both Google and email+password.');
      }
      ...
      if (error.code === 'auth/weak-password') {
        ...
        setError('Password is already linked to this admin account.');
      } else {
        setError(error.message || 'Failed to update admin password');
      }
    } finally {
      setIsUpdating(false);
    }
      ...
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-xl font-semibold text-gray-800">Update Admin Password</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mb-4">
          <p className="text-gray-600 mb-4">
            Add a password to your admin account so you can login with both Google and email+password.
          </p>
        </div>
