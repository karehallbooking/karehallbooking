  ...
  db.collection(collections.bookings)
    .onSnapshot((snapshot) => {
      snapshot.docChanges().forEach(async (change) => {
        ...
        // Skip historical docs created before this server started to avoid re-sending on restarts
      ...
      console.error('❌ Booking watcher stream error (create):', err.message || err);
    });

  // Listen for modifications (status changes)
  db.collection(collections.bookings)
    .onSnapshot((snapshot) => {
      snapshot.docChanges().forEach(async (change) => {
      ...
      console.error('❌ Booking watcher stream error (status):', err.message || err);
    });
}
